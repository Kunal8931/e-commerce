...
class HybridRecommender:
    ...
    def recommend(self, user_id, top_k=10, recent_behavior_ids=[], alpha=0.7):
        ...
        # 7. --- Apply Balanced Category Penalty Mask (CRITICAL FIX) ---
        # Subtracting 0.5 (a moderate penalty) ensures irrelevant items rank low but doesn't
        # zero out the entire list, preventing the 'only one item' issue.
        hybrid_scores[category_irrelevant_mask] -= 0.5

        # 8. Exclude products the user has already purchased 
        hybrid_scores[purchased_indices] = -1
        ...
        for i in topk_indices:
            ...
            # Skip items with negative score (already purchased or penalized too much)
            if score <= 0.05: # Changed from < 0 to <= 0.05 to filter very weak, penalized items
                continue 
